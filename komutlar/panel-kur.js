...
exports.run = async(client, message, args) => {
  ...
  if(panel) return message.channel.send(`Bu sunucuda panel zaten ayarlanmış! Devredışı bırakmak için;  \`${prefix}panel-kur sil\``)
  
  message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Sunucu Panel').setDescription('Gerekli dosyalar kurulsun mu?').setFooter('Onaylıyorsanız 15 saniye içerisinde "evet" yazmalısınız.'))
  .then(() => {
