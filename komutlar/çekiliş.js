...
var filter = m => m.author.id === message.author.id;
      ...
      message.channel.send(`:eight_spoked_asterisk: |D| **Çekilişin yapılacağı kanalın adını yaz **`).then(msg => {
      message.channel.awaitMessages(filter, {
        ...
        msg.edit(':eight_pointed_black_star:| **Çekilişin süresini belirle (1s, 1m, 1h, 1d, 1w)**').then(msg => {
          message.channel.awaitMessages(filter, {
            ...
            if(!collected.first().content.match(/[1-60][s,m,h,d,w]/g)) return message.channel.send(':heavy_multiplication_x:| **Süre dilimi bulunamadı.**');
            duration = collected.first().content
            collected.first().delete();
            msg.edit(':eight_pointed_black_star:| **Ödül belirleyin.**').then(msg => {
              message.channel.awaitMessages(filter, {
                max: 1,
                time: 20000,
                errors: ['time']
                ...
                try {
                  ...
                  .setFooter(message.author.username + " (Destiny çekiliş sistemi)", message.author.avatarURL);
                  message.guild.channels.find("name" , room).send(' :heavy_check_mark: **ÇEKİLİŞ BAŞLADI** :heavy_check_mark:' , {embed: giveEmbed}).then(m => {
                     ...
                     setTimeout(() => {
                       ...
                       .setTitle(title)
                       .setColor("#f558c9")
            .setFooter("(Destiny çekiliş sistemi)")
                     ...
                     m.edit('** 🎉 ÇEKİLİŞ BİTTİ 🎉**' , {embed: endEmbed});
                       ...
                       var embedLel = new Discord.RichEmbed()
                        .setColor("#f558c9")
                        .setDescription("Ödülünü Moderatörleri Etiketleyerek Alabilirsin!").setFooter("(Flex çekiliş sistemi)")
